...
function createMockEvent(method: string, path: string, headers: Record<string, string> = {}) {
  return {
    method,
    path,
    _headers: headers,
...
beforeEach(() => {
  mockGetHeader.mockImplementation((event: any, name: string) => {
...
describe('CSRF middleware', () => {
  ...
  it('blocks POST without X-Requested-With', () => {
    ...
    expect(() => handler(event as any)).toThrow('Forbidden: missing required header')
  })
  ...
  it('blocks PUT without X-Requested-With', () => {
    ...
    expect(() => handler(event as any)).toThrow()
  })
