...
export default defineEventHandler(async (event) => {
    await enforceRateLimit(event, 'stats-update', 10, 60_000)

    const body = await readBody(event)
    const parsed = updateSchema.safeParse(body)
    if (!parsed.success) {
        throw createError({ statusCode: 400, message: 'Invalid stats update payload.' })
    }

    const { status, attempts } = parsed.data
    ...
    // Prevent double-counting: if already played today, skip
