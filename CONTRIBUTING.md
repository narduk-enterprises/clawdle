...
```bash
git clone https://github.com/narduk-enterprises/clawdle.git my-project
cd my-project
pnpm install
...
| --------------- | ---------- | -------------- |
| `clawdle`       | `dev`      | Shared secrets |
| `clawdle`       | `dev_auth` | `example-auth` |

**First-time setup for example-auth:**

```bash
cd apps/example-auth
doppler setup --project clawdle --config dev_auth --no-interactive
```
